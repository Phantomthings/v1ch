...
if "is_ok" not in sess.columns:
    ...
    ok_mask = sess["is_ok"].astype(bool)
    ok = sess.loc[ok_mask].copy()

    if "type_erreur" in sess.columns:
        type_vals = sess["type_erreur"].fillna("")
        energy_mask = ok_mask | (~ok_mask & type_vals.eq("Fin de charge"))
    else:
        energy_mask = ok_mask

    ok_energy = sess.loc[energy_mask].copy()

    if ok_energy.empty:
        st.info("Aucune charge OK ou 'Fin de charge' dans ce périmètre.")
    else:
        dt_s_energy = pd.to_datetime(ok_energy.get("Datetime start"), errors="coerce")
        dt_e_energy = pd.to_datetime(ok_energy.get("Datetime end"), errors="coerce")
        energy = pd.to_numeric(ok_energy.get("Energy (Kwh)"), errors="coerce")

        def date_of_energy(idx):
            if pd.isna(idx) or idx not in ok_energy.index: return "—"
            d = dt_e_energy.loc[idx]
            if pd.isna(d): d = dt_s_energy.loc[idx]
            return d.strftime("%Y-%m-%d %H:%M:%S") if pd.notna(d) else "—"

        def lieu_of_energy(idx):
            if pd.isna(idx) or idx not in ok_energy.index: return "—"
            row = ok_energy.loc[idx]
        ...
        with c3: card("Max (kWh)", f"{e_max_v}", f"{date_of_energy(e_max_i)} — {lieu_of_energy(e_max_i)}")
    ...
    if ok.empty:
        ...
        if d_site.empty:
            ...
            if d_site.empty:
                ...
                with c2: card("Médiane / jour (OK)", f"{med_day}")
